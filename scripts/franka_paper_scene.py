"""
Single-scene PyBullet simulation and paper-figure renderer.

This script runs one Franka Panda pick-and-place trial in a confined
workspace, computes the AABB-based grid-invasion indicator
(`invaded_grid_count`), and exports visualization assets used for the paper.
It is intended as a representative simulation/visualization script, not as the
full batch generator for the complete paper dataset.
"""
...
OUTPUT_DIR = Path(
    os.environ.get(
        "FRANKA_SCENE_OUTPUT_DIR",
        str(PROJECT_ROOT / "outputs" / "paper_scene"),
    )
).expanduser().resolve()


def output_path(*parts):
    """Return an output path under FRANKA_SCENE_OUTPUT_DIR or outputs/paper_scene."""
...
# Paper-scene rendering
PAPER_RENDER_ENABLED  = True
PAPER_RENDER_DIR      = output_path("paper_renders")
PAPER_RENDER_WIDTH    = 2400
PAPER_RENDER_HEIGHT   = 1600
PAPER_VIEW_SECONDS    = 60
PAPER_PROJECT_INVASION_TO_FLOOR = False
PAPER_PRINT_CAMERA    = True
PAPER_CAMERA_EYE      = [-1.55, -1.85, 1.35]
PAPER_CAMERA_TARGET   = [0.20, -0.22, 0.34]
PAPER_CAMERA_UP       = [0.0, 0.0, 1.0]
PAPER_SAFETY_ALPHA    = 0.11
PAPER_SAFETY_EDGE_ALPHA = 0.22
PAPER_SAFETY_MARGIN_STRUCTURES = {
    "fence_left", "fence_right", "ws1_base", "ws2_base", "cylinder"
}
...
FENCE_COLOR             = [0.58, 0.18, 0.14, 1.0]

DEFAULT_PILLAR_X        = -0.33
DEFAULT_PILLAR_Y        = -0.33
CYL_RADIUS              = 0.1
CYL_HEIGHT              = 1.2
...
CYL_COLOR   = [0.42, 0.45, 0.84, 1.0]  # #6B72D6
WS1_COLOR   = [0.47, 0.50, 0.48, 1.0]
WS2_COLOR   = [0.47, 0.50, 0.48, 1.0]
TRAY1_COLOR = [0.91, 0.61, 0.18, 1.0]  # #E89B2E
TRAY2_COLOR = [0.65, 0.42, 0.85, 1.0]  # #A76AD8
BASE_COLOR  = [0.28, 0.28, 0.28, 1.0]

# Pick/place positions are shown with dedicated markers.
TRAY1_COLOR = [0.40, 0.40, 0.45, 1.0]
TRAY2_COLOR = [0.40, 0.40, 0.45, 1.0]
...
def create_visual_box(size, position, color, orientation=None):
...
def create_visual_cylinder(radius, height, position, color):
    vis = p.createVisualShape(
        p.GEOM_CYLINDER,
        radius=radius,
        length=height,
...
def create_paper_floor():
    floor_color = [0.74, 0.74, 0.71, 1.0]
    line_color = [0.60, 0.60, 0.58, 1.0]
    backdrop_color = [0.78, 0.78, 0.76, 1.0]
    floor_id = create_box(
        [8.0, 8.0, 0.02],
        [0.20, -0.10, -0.011],
        floor_color
    ...
    # Visual background panels for publication-style oblique renders.
    create_visual_box([8.0, 0.035, 2.2], [0.20, 3.25, 1.10], backdrop_color)
    create_visual_box([0.035, 8.0, 2.2], [-3.25, -0.10, 1.10], backdrop_color)

    x_min, x_max = -3.60, 3.60
    y_min, y_max = -3.60, 3.60
    spacing = 0.20
    line_w = 0.0035
    z = 0.003
    ...
    x = x_min
    while x <= x_max + 1e-6:
        create_visual_box([line_w, y_max - y_min, 0.002],
                          [x, (y_min + y_max) / 2, z], line_color)
        x += spacing

    y = y_min
    while y <= y_max + 1e-6:
        create_visual_box([x_max - x_min, line_w, 0.002],
                          [(x_min + x_max) / 2, y, z], line_color)
        y += spacing

    return floor_id


def add_dashed_debug_line(start, end, color=[1.0, 0.72, 0.05],
                          line_width=3.0, dash_length=0.045,
                          gap_length=0.030):
    ...
    t = 0.0
    while t < length:
        seg_start = start + unit * t
        seg_end = start + unit * min(t + dash_length, length)
        p.addUserDebugLine(
            seg_start.tolist(),
            seg_end.tolist(),
            lineColorRGB=color,
...
def add_box_outline(center, size, color=[1.0, 0.72, 0.05]):
    cx, cy, cz = center
    sx, sy, sz = size
    ...
    for a, b in edges:
        add_dashed_debug_line(vertices[a], vertices[b], color=color)


def add_cylinder_outline(center, radius, height, color=[1.0, 0.72, 0.05]):
    cx, cy, cz = center
    z0 = cz - height / 2
    z1 = cz + height / 2
    n = 72
    ...
    for zi in [z0, z1]:
        for i in range(n):
            ...
            a0 = 2 * math.pi * i / n
            a1 = 2 * math.pi * (i + 1) / n
            p0 = [cx + radius * math.cos(a0), cy + radius * math.sin(a0), zi]
            p1 = [cx + radius * math.cos(a1), cy + radius * math.sin(a1), zi]
            p.addUserDebugLine(p0, p1, lineColorRGB=color, lineWidth=3.0, lifeTime=0)

    for i in range(0, n, 9):
        a = 2 * math.pi * i / n
        x = cx + radius * math.cos(a)
        y = cy + radius * math.sin(a)
        add_dashed_debug_line([x, y, z0], [x, y, z1], color=color)


def create_safety_margin_3d_outline(env_info, clearance_m):
    safety_color = [1.0, 0.72, 0.05]

    for item in env_info:
        if item.get('name') not in PAPER_SAFETY_MARGIN_STRUCTURES:
            continue

        if item['shape'] == 'box':
            sx, sy, sz = item['size']
            cx, cy, cz = item['position']
            z_min = max(cz - sz / 2 - clearance_m, 0.005)
            z_max = cz + sz / 2 + clearance_m
            outline_size = [sx + 2 * clearance_m,
                            sy + 2 * clearance_m,
                            z_max - z_min]
            outline_center = [cx, cy, (z_min + z_max) / 2]
            add_box_outline(outline_center, outline_size, color=safety_color)

        elif item['shape'] == 'cylinder':
            cx, cy, _ = item['position']
            cz = item['position'][2]
            radius = item['radius']
            height = item['height']
            z_min = max(cz - height / 2 - clearance_m, 0.005)
            z_max = cz + height / 2 + clearance_m
            add_cylinder_outline(
                [cx, cy, (z_min + z_max) / 2],
                radius + clearance_m,
                z_max - z_min,
                color=safety_color
...
def create_safety_margin_translucent_volumes(env_info, clearance_m):
    volume_color = [1.0, 0.82, 0.08, PAPER_SAFETY_ALPHA]
    edge_color = [1.0, 0.62, 0.02, PAPER_SAFETY_EDGE_ALPHA]
    ...
    for item in env_info:
        ...
        if item['shape'] == 'box':
            ...
            volume_center = [cx, cy, (z_min + z_max) / 2]

            create_visual_box(volume_size, volume_center, volume_color)

            # Very subtle solid edges keep the 120 mm volume readable without
            # adding many dotted points.
            ex, ey, ez = volume_size
            edge_w = 0.006
            x0, x1 = cx - ex / 2, cx + ex / 2
            y0, y1 = cy - ey / 2, cy + ey / 2
            z0, z1 = volume_center[2] - ez / 2, volume_center[2] + ez / 2
            for z in [z0, z1]:
                create_visual_box([ex, edge_w, edge_w], [cx, y0, z], edge_color)
                create_visual_box([ex, edge_w, edge_w], [cx, y1, z], edge_color)
                create_visual_box([edge_w, ey, edge_w], [x0, cy, z], edge_color)
                create_visual_box([edge_w, ey, edge_w], [x1, cy, z], edge_color)
...
def create_pick_place_markers(pick_x, pick_y, place_x, place_y):
    pick_color = [0.91, 0.61, 0.18, 1.0]   # #E89B2E
    place_color = [0.65, 0.42, 0.85, 1.0]  # #A76AD8
    tray_cover_color = TRAY1_COLOR

    pick_z = WS1_SIZE[2] + TRAY1_WALL_THICKNESS + 0.006
    place_z = WS2_SIZE[2] + TRAY2_WALL_THICKNESS + 0.006

    # Pick: a flat ring stamped on the tray floor.
    create_visual_cylinder(0.070, 0.006, [pick_x, pick_y, pick_z], pick_color)
    create_visual_cylinder(0.048, 0.007, [pick_x, pick_y, pick_z + 0.001],
                           tray_cover_color)

    # Place: a flat X stamped on the tray floor.
    x_bar_len = 0.130
    x_bar_w = 0.018
    x_bar_h = 0.007
...
def visualize_invaded_grid_points(hazard_grid, radius=0.006,
                                  project_to_floor=True):
    if not hazard_grid.invasion_records:
        return []

    marker_ids = []
    red = [0.90, 0.03, 0.02, 1.0]
    halo = [1.0, 0.16, 0.04, 0.30]

    if project_to_floor:
        xy_points = sorted({
            (round(r['x'], 4), round(r['y'], 4))
            for r in hazard_grid.invasion_records
        })
        for x, y in xy_points:
            marker_ids.append(
                create_visual_cylinder(radius, 0.006, [x, y, 0.028], red)
            )
        return marker_ids

    points = sorted({
        (round(r['x'], 4), round(r['y'], 4), round(r['z'], 4))
        for r in hazard_grid.invasion_records
    })
...
def save_paper_render(path, width=PAPER_RENDER_WIDTH,
                      height=PAPER_RENDER_HEIGHT):
    ensure_parent_dir(path)

    eye = PAPER_CAMERA_EYE
    target = PAPER_CAMERA_TARGET
    distance = float(np.linalg.norm(np.array(eye) - np.array(target)))
    ...
    view_matrix = p.computeViewMatrix(
        ...
        cameraTargetPosition=target,
        cameraUpVector=PAPER_CAMERA_UP
    )
    ...
    try:
        ...
        import matplotlib.pyplot as plt
        plt.imsave(path, rgba[:, :, :3])

    return path


def print_current_debug_camera(label="Current GUI camera"):
    if not PAPER_PRINT_CAMERA:
        return
    try:
        ...
        target = cam[11]
        print(f"\n[{label}]")
        print(f"  cameraDistance={distance:.4f}")
        print(f"  cameraYaw={yaw:.4f}")
        print(f"  cameraPitch={pitch:.4f}")
        print(
            "  cameraTargetPosition="
            f"[{target[0]:.4f}, {target[1]:.4f}, {target[2]:.4f}]"
        )
    except Exception as exc:
        print(f"[Camera] Could not read debug camera values: {exc}")


def hold_paper_view(seconds, dt):
    if seconds <= 0:
        return
    print_current_debug_camera("GUI camera at render hold start")
    for _ in range(int(seconds / dt)):
        p.stepSimulation()
        time.sleep(dt)
    print_current_debug_camera("GUI camera after manual adjustment")
...
def run_simulation(layout_params=None, trial_params=None,
    ...
    """Run one trial and export optional paper-scene visualizations."""
    ...
    if headless:
        ...
        #     cameraYaw=-135,
        #     cameraPitch=-35,
        #     cameraTargetPosition=PAPER_CAMERA_TARGET,
        # )
    ...
    # Paper-render floor: use a custom matte tile floor instead of plane.urdf
    # to avoid the default blue/white grid texture and z-fighting.
    create_paper_floor()
    ...
    if PAPER_RENDER_ENABLED:
        create_pick_place_markers(cube_x, cube_y, place_x, place_y)
    ...
    if PAPER_RENDER_ENABLED:
        create_safety_margin_translucent_volumes(
            env_info,
            MIN_CLEARANCE_MM / 1000.0
        )
    ...
    if PAPER_RENDER_ENABLED:
        visualize_invaded_grid_points(
            hazard_grid,
            radius=0.007,
            project_to_floor=PAPER_PROJECT_INVASION_TO_FLOOR
        )
        paper_scene_path = os.path.join(
            PAPER_RENDER_DIR,
            f"scene_{layout_id:03d}_trial_{trial_id:03d}.png"
        )
        save_paper_render(paper_scene_path)
        if verbose:
            print(f"  Saved paper scene render: {paper_scene_path}")
        if not headless:
            hold_paper_view(PAPER_VIEW_SECONDS, DT)

    # Layout images are exported separately from the high-resolution scene render.
    layout_image_path = None
    paper_xy_path = os.path.join(
        LAYOUT_IMAGE_DIR,
        f"layout_{layout_id:03d}_trial_{trial_id:03d}_xy_safety_margin.png",
    )
    save_xy_safety_margin_figure(layout_params, trial_params, paper_xy_path)
...
if __name__ == "__main__":
    print("="*60)
    print("Franka Panda paper-scene renderer")
